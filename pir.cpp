...
void VLHEPIR::Update(Matrix& D, Matrix& H, Matrix& D_, Matrix& H_, Matrix& A, UpdateType updateType, Matrix& array, uint64_t* index)
{
    DBUpdate(D, D_, updateType, array, index);
    StateUpdate(H, H_, D_, A, updateType, index, array.rows);
}

void VLHEPIR::DBUpdate(Matrix& D, Matrix& out, UpdateType updateType, Matrix& array, uint64_t* index)
{//m修改的数量
    uint64_t m = array.rows;
    if (updateType == MODIFY)//modify
    {
        memcpy(out.data, D.data,index[0] * sizeof(uint64_t));
        for (int i = 0; i < m - 1; i++)
        {
            memcpy(out.data + index[i] * D.cols, array.data + i * D.cols,D.cols * sizeof(Elem));
            memcpy(out.data + (index[i] + 1) * D.cols,D.data + (index[i] + 1) * D.cols,D.cols * (index[i + 1] - index[i] - 1) * sizeof(Elem));
        }
        memcpy(out.data + index[m - 1] * D.cols, array.data + (m - 1) * D.cols,D.cols * sizeof(Elem));
        memcpy(out.data + (index[m - 1] + 1) * D.cols, D.data + (index[m - 1] + 1) * D.cols, D.cols * (D.rows - index[m - 1] - 1) * sizeof(Elem));
    }
    else if (updateType == INSERT)//insert
    {//insert位置
        ell = ell + m;
        memcpy(out.data, D.data, D.rows * D.cols * sizeof(Elem));
        memcpy(out.data + D.rows * D.cols, array.data, m * array.cols * sizeof(Elem));

    }
    else if (updateType == DELETE)//delete
    {
        ell = ell - m;
        memcpy(out.data, D.data,index[0] * D.cols * sizeof(Elem));
        for (int i = 0; i < m - 1; i++)
        {
            memcpy(out.data + (index[i] - i) * D.cols,D.data + (index[i] + 1) * D.cols,(index[i + 1] - index[i] - 1) * D.cols * sizeof(Elem));
        }
        memcpy(out.data + (index[m - 1] - m + 1) * D.cols, D.data + (index[m - 1] + 1) * D.cols, (D.rows - index[m - 1] - 1) * D.cols * sizeof(Elem));
    }
}

void VLHEPIR::StateUpdate(Matrix& H, Matrix& out, Matrix& D, Matrix& A, UpdateType updateType, uint64_t* index, uint64_t m)
{
    if (updateType == MODIFY)//modify
    {
        memcpy(out.data, H.data, index[0] * sizeof(Elem));
        for (int i = 0; i < m - 1; i++)
        {
            uint64_t tmpD = index[i] * D.cols;
            uint64_t tmpH = index[i] * H.cols;
            ...
            for (size_t k = 0; k < D.cols; k++) {
                for (size_t j = 0; j < A.cols; j++) {
                    out.data[tmpH + j] += D.data[tmpD + k] * A.data[A.cols * k + j];
                }
            }
            memcpy(out.data + (index[i] + 1) * H.cols,H.data + (index[i] + 1) * H.cols,H.cols * (index[i + 1] - index[i] - 1) * sizeof(Elem));
        }
        uint64_t tmpD = index[m - 1] * D.cols;
        uint64_t tmpH = index[m - 1] * H.cols;
        for (int j = 0; j < H.cols; j++)
        {
            out.data[tmpH + j] = 0;
        }
        ...
        memcpy(out.data + (index[m - 1] + 1) * H.cols, H.data + (index[m - 1] + 1) * H.cols, H.cols * (H.rows - index[m - 1] - 1) * sizeof(Elem));
    }
    else if (updateType == INSERT)
    {
        memcpy(out.data, H.data, H.rows * H.cols * sizeof(Elem));
        for (int i = 0; i < m; i++)
        {
            uint64_t tmpD = (D.rows - m) * D.cols + i * D.cols;
            uint64_t tmpH = (D.rows - m) * H.cols + i * H.cols;
    ...
    {
        memcpy(out.data, H.data,index[0] * H.cols * sizeof(Elem));
        for (int i = 0; i < m - 1; i++)
        {
            memcpy(out.data + (index[i] - i) * H.cols,H.data + (index[i] + 1) * H.cols,(index[i + 1] - index[i] - 1) * H.cols * sizeof(Elem));
        }
        memcpy(out.data + (index[m - 1] - m + 1) * H.cols, H.data + (index[m - 1] + 1) * H.cols, (H.rows - index[m - 1] - 1) * H.cols * sizeof(Elem));
    }
}
